...
import rule.rule1 as r1
import rule.rule2 as r2
import rule.rule5 as r5
...
def train(args, model, device, train_loader, optimizer, epoch, criterion):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        output = model(Variable(data, requires_grad=True))  # 包装张量
...
def create_hook(output_dir, module=None, hook_type="saveall"):
    ...
    if hook_type == "saveall":
        hook = Hook(
            out_dir=output_dir,
            save_config=SaveConfig(save_steps=[i * 10 for i in range(40)]),
            save_all=True,
        )
...
def main():
    ...
    print(r1.input_balance(train_loader))
    print("Not_Normalized_Data Result:", end=" ")
    print(r2.Not_Normalized_Data(train_loader))
    ...
    print(r5.Classifier_Confusion(10,labels,predictions))
